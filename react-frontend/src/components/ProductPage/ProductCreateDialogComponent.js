...
const ProductCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            name: _entity.name,
            manufacture: _entity.manufacture,
        };
    ...
    return (
        <Dialog header="Create" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            <div>
                <p className="m-0">Manufacture:</p>
                <InputText className="w-full mb-3" value={_entity?.manufacture} onChange={(e) => setValByKey("manufacture", e.target.value)}  />
            </div>
